...
export default class ResourceSwapper extends Module {
    ...
    main() {
        ...
        Manager.registerBeforeRequestCallback((details, callback) => {
            ...
            if(url.hostname !== 'krunker.io' || !existsSync(join(this.path, url.pathname)) || !statSync(join(this.path, url.pathname)).isFile()) return callback({ cancel: false });

            return callback({ redirectURL: 'client-swapper://' + url.pathname });
        });
    }
}